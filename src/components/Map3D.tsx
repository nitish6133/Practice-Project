...
export const Map3D: React.FC<Map3DProps> = ({
  ...
  const optimizeThreeJSRendering = useCallback(() => {
    if (threeManagerRef.current && threeManagerRef.current.meshes.size > 100) {
      threeManagerRef.current.meshes.forEach((mesh) => {
  ...
  const debouncedOptimizeThreeJSRendering = useCallback(debounce(optimizeThreeJSRendering, 500), [optimizeThreeJSRendering]);
  ...
  useEffect(() => {
    ...
    const debouncedMapMove = debounce(() => {
      if (map.current) {
        map.current.repaint = false;
        debouncedOptimizeThreeJSRendering();
      }
    }, 100);
    ...
    map.current.on('zoom', debouncedMapMove);
    map.current.on('move', debouncedMapMove);
